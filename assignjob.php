        ...
        <div class="row">
                                    ...
                                    <div class="col-md-6 ">
                                        ...
                                        <div class="form-floating form-floating-outline mb-3 mt-5">
                                            <div class="card-body p-0">
                                                ...
                                                <label for="JobFiles" class="mb-0">Choose files:</label>
                                        ...
                                        <div class="form-floating form-floating-outline mb-3 mt-5">
                                            <input type="number" id="JobFileCount" class="form-control"
                                                placeholder="Enter a Page Count" pattern="\d{4}" maxlength="4">
         ...
         <script>
            function validateFile() {
                ...
                const fileError = $('#fileError');
                const allowedExtensions = /\.(jpg|jpeg|png|webp|pdf)$/i;
                const maxSizeInBytes = 55 * 1024 * 1024; // ✅ 50 MB
                const maxFilesAllowed = 20; // ✅ limit
                ...
                if (files.length > maxFilesAllowed) {
                    fileError.text(`❌ You can upload maximum ${maxFilesAllowed} files only.`).show();
                    $('#JobFiles').val(''); // Clear input
                    return false;
                }
                 let totalSize = 0;
                ...
                for (let i = 0; i < files.length; i++) {
                    const file = files[i];
                      totalSize += file.size; // ✅ add size to total
                    ...
                    if (file.name.length > 50) {
                        fileError.text(`"${file.name}" has too long name. Limit is 50 characters only allowed.`).show();
                        $('#JobFiles').val('');
                        return false;
                    }
                    ...
                    // ✅ Check file size
                    if (file.size > maxSizeInBytes) {
                        fileError.text(`"${file.name}" exceeds 200MB limit.`).show();
                        $('#JobFiles').val('');
                        return false;
                    }
                }

                  // ✅ Final check: total size of all files
                    if (totalSize >= maxSizeInBytes) {
                        fileError.text(`Total file size exceeds 50MB limit.`);
                        fileError.show();
                        fileInput.value = ''; // Clear input
                        return false;
                    }
