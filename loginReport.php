    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row">
                            ...
                            <div class="card-body">
                                <div class="row gy-5">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="row mb-3">
                                            <h5 class="Address_ship_headline">Log Report</h5>
                                        </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.getElementById('reporlogForm').addEventListener('submit', function(e) {
                    e.preventDefault();
                    ...
                    const formData = new URLSearchParams({
                        ...
                        'submit': 'logReport' // Ensure this is a string
                    });
                ...
                //     // Prepare headers
                //     const headers = [
                //         'S.No', 'Name', 'Date', 'From Time', 'To Time', 'Status', 'Designation'
                ...
                //             'From Time': formatTime(row.fromtime || ''),
                //             'To Time': row.totime ? formatTime(row.totime) :
                ...
                //         '0'); // Months are 0-indexed, so adding 1
                //         let year = currentDate.getFullYear();
                //         let formattedDate = `${day}-${month}-${year}`;

                //         // Get the log name (or default to 'User Log')
                //         const logName = exportData.length > 0 ? exportData[0]['Name'] : 'User Log';

                //         // Append the formatted date to the filename
                //         const fileName = `${logName}-Log Report-${formattedDate}.xlsx`;
                ...
                .destroy(); // Clear and destroy any existing DataTable instance
                    ...
                    const columnDefs = [{
                        ...
                        },
                        {
                            targets: 4,
                            render: function(data, type, full) {
                                // Convert fromtime to am/pm format
                                const fromTimeParts = full['fromtime'].split(
                                ':'); // Split the time string
                                const fromHours = parseInt(fromTimeParts[0], 10);
                                const fromMinutes = fromTimeParts[1];
                                const fromPeriod = fromHours >= 12 ? 'pm' : 'am';
                                const formattedFromTime = ((fromHours % 12) || 12) + ':' + fromMinutes +
                                    ' ' + fromPeriod;

                                return '<span class="text-heading">' + formattedFromTime + '</span>';
                            }
                        },
                        {
                            targets: 5,
                            render: function(data, type, full) {
                                // Check if totime is null
                                if (!full['totime']) {
                                    return '<span class="text-heading">Currently Active</span>';
                                }

                                // Split totime for formatting
                                const toTimeParts = full['totime'].split(':'); // Split the time string
                                const toHours = parseInt(toTimeParts[0], 10);
                                const toMinutes = toTimeParts[1];

                                // Determine AM/PM
                                const period = toHours >= 12 ? 'pm' : 'am';
                                const formattedToTime = ((toHours % 12) || 12) + ':' + toMinutes + ' ' +
                                    period;

                                return '<span class="text-heading">' + formattedToTime + '</span>';
                            }
                        },

                        {
                            ...
                            render: function(data, type, full) {
                                ...
                                return '<span class="text-heading">' + (status == 1 ? 'Active' :
                                    'Inactive') + '</span>';
                            }
                        },
                        {
