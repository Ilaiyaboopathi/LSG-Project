...
require_once 'include/handlers/upload_view.php';

// Define the path to your service account credentials
$credentialsPath = 'credentials.json'; 

// Define the folder ID
$folderId = '10hqIlAD29RwlUNlQLNvUKu0bAg7ekE9t'; // Replace with your folder ID

// Create an instance of the GoogleDriveAPI class
$googleDrive = new GoogleDriveAPI($credentialsPath, $folderId);

// Fetch all files from the specified folder
$googleDrive->fetchFiles();

// Get the result (all files fetched from Google Drive)
$files = $googleDrive->getResult();


// Save the result to a JSON file
$googleDrive->saveToJson('assets/json/drive_docs_upload.json');
                            ...
                            <div class="row">
                                <div class="col-6 ">
                                    ...
                                    <div class="col-md-12 mb-4">
                                        <div class="form-floating form-floating-outline">
                                            ...
                                            <input type="text" id="docs_name" class="form-control"
                                                ...
                                                maxlength="50" placeholder="Document Name" required name="docs_name" />
                                            <label for="docs_name">Document Name *</label>
                                            <small id="error-message" style="color:red; display:none;">Please enter a
                                                document name (max 50 characters).</small>
                                        </div>
                                                  
                                    </div>
                                    ...
                                    <div class="col-12 mb-4">
                                        ...
                                        <input type="hidden" id="file_upload_hidden" required
                                            value="<?php echo empty($file_url) ? '' : htmlspecialchars($file_url); ?>">
                                        <input class="form-control" type="file" id="Emp_file_upload"
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-datatable table-responsive">
                            <table class="drive_data_tables_docs_upload table">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th>S.No</th>
...
<script src="assets/js/drive_docs_upload.js"></script>
...
$(document).ready(function() {
        ...
        reader.onload = function(e) {
            ...
            if (fileExtension === 'png' || fileExtension === 'jpg' || fileExtension === 'jpeg') {
                ...
                switch (fileExtension) {
                    case 'pdf':
                        $('#preview_emp_file').attr('src', 'assets/img/illustrations/pdf.png')
                            .show();
                        $('#preview_emp_file').addClass('view_custom_img');
                        break;
                    case 'xlsx':
                        $('#preview_emp_file').attr('src', 'assets/img/illustrations/xlsx.png')
                            .show();
                        $('#preview_emp_file').addClass('view_custom_img');
                        break;
                    case 'csv':
                        $('#preview_emp_file').attr('src', 'assets/img/illustrations/csv.png')
                            .show();
                        $('#preview_emp_file').addClass('view_custom_img');
                        break;
                    case 'docx':
                        $('#preview_emp_file').attr('src', 'assets/img/illustrations/docs.png')
                            .show();
                        $('#preview_emp_file').addClass('view_custom_img');
                        break;
                    case 'doc':
                        ...
                        break;
                    default:
                        $('#preview_emp_file').attr('src', 'assets/img/illustrations/No_file.png')
                            .show();
                        $('#preview_emp_file').addClass('view_custom_img');
                }
            }
    ...
    $('#file_docs_submit').on('submit', function(e) {
        ...
        if (!name || !file || !select_employees) {
            //showModalWithParams('Please fill in all mandatory fields.', 'false');
            $('#SameNameAlert').show();
        ...
        if (name.length > 60) {
            showModalWithParams('File name must be 60 characters or less.', 'false');
            return;
        }
        ...
        if (file) {
            ...
            if (!allowedExtensions.includes(fileExtension)) {
                showModalWithParams(
                    'Allowed file types are: pdf, csv, doc, docx, jpg, png, jpeg, xlsx.', 'false');
                return;
            }
        }
        ...
        $.ajax({
            url: 'include/handlers/upload_drive_req.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
                ...
                if (trimmedResponse === 'success') {
                    showModalWithParams(`$(name) File Added.`, 'true');
                    //window.location.href = 'Documents.php?page=Documents';
...
function copyurl(event, urlToCopy) {
    ...
    // Optionally, show a message indicating the URL has been copied
...
function validateFile() {
    const fileInput = document.getElementById('Emp_file_upload');
    const fileError = document.getElementById('fileError');
    const allowedExtensions = /(\.doc|\.docx|\.xlsx|\.xls|\.pdf|\.txt|\.jpg|\.jpeg|\.png|\.zip)$/i;
    ...
    if (file) {
        ...
        if (!allowedExtensions.exec(file.name)) {
            fileError.textContent =
                'Invalid file format. Please upload a .doc, .docx, .xlsx, .xls, .pdf, .png, .jpg, .jpeg or .txt file.';
            fileError.style.display = 'block';
            fileInput.value = ''; // Clear the input
            return;
        }
